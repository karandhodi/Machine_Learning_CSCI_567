...
def get_k_means_plus_plus_center_indices(n, n_cluster, x, generator=np.random):
    ...
    for k in range(0, n_cluster):
        D2 = np.array([min([np.inner(c - x1, c - x1) for c in C]) for x1 in x])
        probs = D2 / D2.sum()
        #print(probs)
        ...
        for j, p in enumerate(cumprobs):
            if r < p:
                i = j
                break
        #np.concatenate(C, x[i])
        C=np.append(C,x[i])
        centers=np.append(centers,i)
        print(centers)
...
class KMeans():
    ...
    def fit(self, x, centroid_func=get_lloyd_k_means):
        ...
        mu = x[centroid_func(len(x), self.n_cluster, x, self.generator), :]
